...
def test_get_cpu_info(mock_cpu_count, mock_cpu_freq, mock_processor):
    ...
    with patch(
        "builtins.open", new_callable=mock_open, read_data=cpu_info_text
    ) as mock_file:
...
@pytest.mark.skipif(
    not hasattr(psutil, "sensors_temperatures"),
    reason="psutil.sensors_temperatures is not available on this platform",
)
def test_get_system_temperature():
    with patch("psutil.sensors_temperatures") as mock_sensors_temperatures:
        # Create mock sensor reading
