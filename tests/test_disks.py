...
def test_get_disk_space(mock_disk_partitions, mock_disk_usage):
    mock_disk_partitions.return_value = [
        psutil._common.sdiskpart(device='/dev/sda1', mountpoint='/', fstype='ext4', opts='rw,relatime', maxfile=4096, maxpath=4096)
    ]
    mock_disk_usage.return_value = psutil._common.sdiskusage(total=100, used=50, free=50, percent=50)
