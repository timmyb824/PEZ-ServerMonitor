...
from sysinformer.core.latency import (  # replace 'sysinformer.latency' with actual module name
...
def test_check_ping():
    with patch("sysinformer.core.latency.ping", return_value=0.002):
        assert check_ping("localhost") == "2.0 ms"  # 0.002 seconds = 2 milliseconds

    with patch("sysinformer.core.latency.ping", return_value=None):
...
#     mock_check_ping = mocker.patch('sysinformer.latency.check_ping', side_effect=[1, 'Timed Out', 2])
...
def test_perform_ping(mocker):
    mock_check_ping = mocker.patch(
        "sysinformer.core.latency.check_ping", side_effect=[1, "Timed Out", 2]
    )
    ping_hosts = ["host1", "host2", "host3"]
...
@patch("sysinformer.core.latency.parse_config_file")
@patch("sysinformer.core.latency.perform_ping")
@patch("sysinformer.core.latency.calculate_average_latency")
def test_print_latency_info(
    ...
    mock_parse_config_file.return_value = {"ping_hosts": ["host1", "host2", "host3"]}
